...
import aslrDataset_imitation_128
...
os.makedirs("images_imitation_128", exist_ok=True)

#TODO: Improve the Generator and the Discriminator layers

def build_generator(latent_size):
    """Building the generator
    
    Arguments:
        latent_size {int} -- [description]
    
    Returns:
        [Model([latent, image_class], fake_image)] -- [description]
    """
    ...
    cnn.add(Dense(4 * 4 * 256, input_dim=latent_size, activation='relu'))
    cnn.add(Reshape((4, 4, 256)))

    # upsample to (8, 8, ...)
    cnn.add(Conv2DTranspose(128, 5, strides=1, padding='valid',
                            activation='relu',
    ...
    # upsample to (32, 32, ...)
    cnn.add(Conv2DTranspose(32, 5, strides=2, padding='same',
                            activation='relu',
                            kernel_initializer='glorot_normal'))
    cnn.add(BatchNormalization())

    # upsample to (64, 64, ...)
    cnn.add(Conv2DTranspose(16, 5, strides=2, padding='same',
                            activation='relu',
                            kernel_initializer='glorot_normal'))
    cnn.add(BatchNormalization())
...
if __name__ == '__main__':
    ...
    (x_train, y_train), (x_test, y_test) = aslrDataset_imitation_128.create_training_data()
    ...
    for epoch in range(1, epochs + 1):
        ...
        #TODO: Seperate the generated images to the classes
        ...
        if (epoch % 10) == 0:
            ...
            img = (np.concatenate([r.reshape(-1, img_size)
                                for r in np.split(img, num_classes+1)
                                ], axis=-1) * 127.5 + 127.5).astype(np.uint8)

            Image.fromarray(img).save(
                'images_imitation_128/plot_epoch_{0:03d}_generated.png'.format(epoch))
